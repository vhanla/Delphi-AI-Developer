...
uses
  SysUtils, StrUtils, Classes, Character, TypInfo, Math, CodeToHtml;
...
uses MarkdownTables, MarkdownMathCode, MarkdownCodeCollector;
...
//          list.add(line.value.substring(4)) P{STfix
  ...
  begin
    ...
    try
      ...
      SourceCodeCollection.Add(code.Text);
      code.Text := TCodeToHtml.Convert(code.Text);
      out_.Append('</code></pre><input type="button" value="✏" onclick="'+
        Format('%d',[SourceCodeCollection.GetCount])+'"/>'+code.Text+'<pre><code>');
    finally
      code.Free;
    end;
    {while (line <> nil) do
